...
from app.models.all_models import Fault, FaultAttachment, KnowledgeBase, Project
...
def get_dashboard_stats(
    ...
    faults_with_actions = (
        db.query(Fault)
        .filter(Fault.planned_actions.isnot(None), Fault.planned_actions != "")
        .count()
    )
    ...
    projects_list = [
        {"id": p.id, "name": p.name, "client": p.client, "faults_count": p.faults_count}
    ...
    recent_faults = (
        ...
        .order_by(
            ...
            case(
                (Fault.updated_at.isnot(None), Fault.updated_at), else_=Fault.created_at
            ).desc()
        )
        .limit(5)
        .all()
        ...
        recent_faults_list.append(
            {
                ...
                if fault.updated_at
                else fault.created_at.isoformat(),
            }
        )
