...
def export_faults_pdf(
    ...
    limit: int = Query(
        5000, ge=1, le=10000, description="Максимальное количество записей"
    ),
    ...
    try:
        ...
        if not faults:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Нет данных для экспорта"
            )
...
def export_single_fault_pdf(
    ...
    try:
        ...
        if fault.clones:
            clones_data = [
                {"id": c.id, "title": c.title or "Без названия"} for c in fault.clones
            ]
