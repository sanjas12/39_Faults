...
def update_article(
    ...
    if 'related_faults' in update_data:
        old_ids = [int(id.strip()) for id in old_related_faults.split(',') if id.strip()] if old_related_faults else []
        new_ids = [int(id.strip()) for id in article.related_faults.split(',') if id.strip()] if article.related_faults else []
        ...
        for fault_id in old_ids:
            if fault_id not in new_ids:
                ...
                if fault:
                    fault_ids = [int(id.strip()) for id in fault.linked_knowledge_ids.split(',') if id.strip()] if fault.linked_knowledge_ids else []
                    if article_id in fault_ids:
                        fault_ids.remove(article_id)
                        fault.linked_knowledge_ids = ','.join([str(id) for id in fault_ids]) if fault_ids else None
                        db.add(fault)
            ...
            if fault_id not in old_ids:
                ...
                if fault:
                    ...
                    if article_id not in fault_ids:
                        ...
                        fault.linked_knowledge_ids = ','.join([str(id) for id in fault_ids])
                        db.add(fault)
    ...
    if article.related_faults:
        ...
        if ids:
            faults = db.query(Fault).filter(Fault.id.in_(ids)).all()
            linked_faults = [
                {"id": f.id, "title": f.title, "status": f.status, "severity": f.severity}
                for f in faults
            ]
